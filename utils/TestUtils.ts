...
export async function generateMerkleTreeAndKeys(numLeaves: number): Promise<MerkleTreeAndSource> {
    ...
    for (let i = 0; i < numLeaves; i++) {
        ...
        leaves.push(await pedersenHashConcat(leafNullifiers[i], leafSecrets[i]));
    }
...
export async function addNewCommitment (filename: string, newcommitment: string, treeheight: number): Promise<MerkleTree> {
...
export async function getMerkleTreeFromPublicListOfCommitments (filename: string, treeheight: number): Promise<MerkleTree> {
    ...
    // replace and empty leaf with the newCommitment leaf 
        ...
        for (let i = commitments.length; i < 2 ** treeheight; i++) {
            commitmentsBigInt.push(randomBigInt(31));
            console.log("add random to merkle tree")
        }
