...
describe("Zeko Airdrop", function () {
  ...
  //   let ERC721_ADDR = "0xF45Da6e3528499dCE377f7fbD970a74d2970b1Bd"; // TO MODIFTY
  //   zekoNFT = await hre.ethers.getContractAt("ZekoGenerativeNFT", ERC721_ADDR);
  ...
  before(async function () {
    ...
    // zekoNFT = await hre.ethers.getContractAt("ZekoGenerativeNFT", ERC721_ADDR);
    // privateAirdrop = await hre.ethers.getContractAt("PrivateAirdrop", AIRDROP_ADDR);
  });
  ...
  it("should allow a user that provides a valid proof to collect an NFT from the airdrop contract", async function () {
    ...
    validProof =
        await generateProofCallData(
            ...
            WASM_BUFF,
            ZKEY_BUFF);
    validNullifierHash = toHex(await pedersenHash(nullifier)); // hash of the nullifier => need to be passed to avoid double spending
  ...
  //   let invalidNullifierHash = "0x18659e56dc8c7befc9080154494b4167ba42e3b1ffc9390b222abd8f2d00223d"
  //   await expect(privateAirdrop.connect(collector).collectAirdrop(invalidProof, invalidNullifierHash)).to.be.reverted
  // });
